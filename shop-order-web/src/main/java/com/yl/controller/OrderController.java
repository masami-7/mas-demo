...
public class OrderController {
    ...
    public ModelAndView submitOrder(String receiveAddressId, BigDecimal totalAmount, String tradeCode, HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap) {
        ...
        if (success.equals("success")) {
            ...
            // 重定向到支付系统
            ModelAndView mv = new ModelAndView("redirect:http://localhost:9000/index");
    ...
    public String toTrade(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap) {
        ...
        List<OmsCartItem> omsCartItemsFromCookie = new ArrayList<>();
        // 合并cookie和用户数据库
        String cartListCookie = CookieUtil.getCookieValue(request, "cartListCookie", true);
        if (StringUtils.isNotBlank(cartListCookie)) {
            omsCartItemsFromCookie = JSON.parseArray(cartListCookie, OmsCartItem.class);
            if (omsCartItems.size() > 0) {
                for (OmsCartItem cartItemFromCookie : omsCartItemsFromCookie) {
                    for (OmsCartItem cartItem : omsCartItems) {
                        if (cartItemFromCookie.getProductSkuId().equals(cartItem.getProductSkuId())) {
                            cartItem.setQuantity(String.valueOf(Integer.valueOf(cartItem.getQuantity()) + Integer.valueOf(cartItemFromCookie.getQuantity())));
                        }
                    }
                }
            } else {
